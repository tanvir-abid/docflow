// ─── Selection Toolbar ────────────────────────────────────────────────────────
import { initSelectionToolbar } from './modules/selection-toolbar.js';
...
import { PrintTool }         from './modules/print.js';
...
const DEFAULT_CONFIG = {
  ...
  selectionToolbar: true,   // set false to disable the floating selection toolbar
  onToast: null,            // callback(message: string) — fired when the toolbar needs to show a toast
};
...
export class DocFlowEditor {
   ...
   * Adds the tool to the registry and immediately rebuilds the toolbar
   * in-place — no need to call _buildTools() or _buildUI() manually, and
   * the content area / status bar are left completely untouched.
   *
   * The tool's name must also appear in `config.tools` (or DEFAULT_TOOL_ORDER
   * when config.tools is null) for it to be visible in the toolbar.
   *
   * @param {Object} toolModule  - Tool object with at least { name, createButton, execute }
   */
  registerTool(toolModule) {
    ...
    // If the editor is already mounted (toolbar exists in the DOM), rebuild
    // the toolbar in-place so the new tool appears immediately.
  ...
  /**
   * Rebuilds the toolbar's child nodes from this._activeTools.
   * Safe to call multiple times — clears existing buttons first.
   * Never touches contentArea, statusBar, or the outer root element.
   */
  _renderToolbar() {
    // Clear previous buttons and dividers
  ...
  _initSelectionToolbar() {
    if (!this.config.selectionToolbar) return;

    this._selectionToolbarHandle = initSelectionToolbar(this, {
      onToast: this.config.onToast ?? null,
    });
  }
  ...
  destroy() {
    this._selectionToolbarHandle?.destroy();
    this._selectionToolbarHandle = null;

/**
 * Background Color (Highlight) Tool Module
 * Toolbar button opens a color panel with presets, last-used swatch,
 * and a native custom color input. Applies highlight color to selection.
 */
export const BgColorTool = {
  name: 'bgColor',
  ...
  _presets: [
    ...
    '#fed7aa', '#e9d5ff', '#fbcfe8', '#99f6e4',
    // Row 2 — deeper tones
    '#fde047', '#4ade80', '#60a5fa', '#f87171',
  ...
  icon(color) {
    // Marker / highlighter pen icon with color bar
    return `<svg width="16" height="16" viewBox="0 0 24 24" fill="none"
        stroke="currentColor" stroke-width="2"
        stroke-linecap="round" stroke-linejoin="round">
      <path d="M15.5 3.5L20.5 8.5L9 20H4V15L15.5 3.5Z"/>
      <path d="M13 6L18 11"/>
  ...
  _buildPanel() {
    ...
    lastSwatch.addEventListener('click', () => {
      this._pick(this._lastColor);
    });
    ...
    // Colour preview swatch that also opens the native picker
    ...
    noneBtn.type = 'button';
    noneBtn.textContent = '✕  Remove highlight';
    ...
    noneBtn.addEventListener('click', () => this._pick('transparent'));
  ...
  _togglePanel(wrapper) {
    ...
    // Sync hex input & custom preview to last color
  ...
  _applyColor(editor, color) {
    ...
    if (!applied) this._applyViaSpan(sel, 'backgroundColor', color);
